...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Formulir Edit Surat Keluar</h6>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('surat-keluar.update', $suratKeluar->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('nomor_surat') is-invalid @enderror" 
                               id="nomor_surat" name="nomor_surat" value="{{ old('nomor_surat', $suratKeluar->nomor_surat) }}" required>
                        @error('nomor_surat')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggal_surat') is-invalid @enderror" 
                               id="tanggal_surat" name="tanggal_surat" value="{{ old('tanggal_surat', $suratKeluar->tanggal_surat->format('Y-m-d')) }}" required>
                        @error('tanggal_surat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('tujuan') is-invalid @enderror" 
                           id="tujuan" name="tujuan" value="{{ old('tujuan', $suratKeluar->tujuan) }}" required>
                    @error('tujuan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('klasifikasi_arsip_id') is-invalid @enderror" id="klasifikasi_arsip_id" name="klasifikasi_arsip_id">
                            ...
                            @foreach($klasifikasis as $klasifikasi)
                                <option value="{{ $klasifikasi->id }}" {{ old('klasifikasi_arsip_id', $suratKeluar->klasifikasi_arsip_id) == $klasifikasi->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    <label class="form-label">File Scan Saat Ini</label>
                    <div class="alert alert-info">
                        <a href="{{ Storage::url($suratKeluar->file_scan) }}" target="_blank" class="btn btn-sm btn-info">
                ...
                <div class="mb-3">
                    <label for="file_scan" class="form-label">Upload File Scan Baru (PDF)</label>
                    <input type="file" class="form-control @error('file_scan') is-invalid @enderror" 
                           id="file_scan" name="file_scan" accept=".pdf">
                    <small class="text-muted">Format: PDF, Maksimal 5MB. Kosongkan jika tidak ingin mengganti file.</small>
                    @error('file_scan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Update Surat Keluar
                    </button>
                </div>
            </form>
        </div>
